...
def main():
    accelerator.init_trackers(
        project_name=f'{configs.project}',
        init_kwargs={
            'wandb':
            {
                'tags': [f'{configs.dataset}', f'{configs.model}'],
                'group': f'{configs.dataset}',
                'name': f'{configs.model}-{configs.dataset}-{datetime.now()}',
                'config': configs,
            }
        }
    )
    ...
    with accelerator.main_process_first():
        ...
        accelerator.print('Tokenizer loaded. Tokenizing the data...' + '=' * 50)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-project', type=str, default='Lora_Deepspeed')
    ...
    # accelerate args
    parser.add_argument('-per_batch_size', default=4, type=int, help='per device batch size')
    parser.add_argument('-accumulation_steps', default=2, type=int, help='')
    parser.add_argument('-warmup_steps', default=100, type=int, help='')
    parser.add_argument('-warmup_ratio', default=1e-3, type=float, help='')
    parser.add_argument('-weight_decay', default=0.01, type=float, help='')
    parser.add_argument('-optim', default='adamw_torch', type=str, help='')
    ...
    accelerator = Accelerator(log_with='wandb', gradient_accumulation_steps=configs.accumulation_steps)
    main()
